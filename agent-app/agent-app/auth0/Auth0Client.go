...
const (
	USER_ROLE_ID = "rol_wo3otT9ab1Mbg6P4"
)

const (
	USER_AGENT_ROLE_ID = "rol_wo3otT9ab1Mbg6P4"
)

const (
	OWNER_ROLE_ID = "rol_JIW9eY3T06SBxkEL"
)

type Auth0Client interface {
	Register(email string, password string) (string, error)
	Update(email string, auth0ID string) error
	UpdateRole(auth0ID string) error
...
func (c *auth0Client) Register(email string, password string) (string, error) {
	...
	b, _ := json.Marshal(&RegistrationRequest{Email: email, Password: password, Connection: "AgentApp-Users"})
...
func (c *auth0Client) Update(email string, auth0ID string) error {
	...
	b, _ := json.Marshal(&UpdateRequest{Email: email, Connection: "AgentApp-Users"})
...
func (c *auth0Client) UpdateRole(auth0ID string) error {
	apiToken, err := c.getAPIToken()
	...
	// with this endpoint, user role cannot be set
	endpoint := fmt.Sprintf("https://%s/api/v2/users/%s/roles", c.domain, auth0ID)

	b, _ := json.Marshal(&RoleRequest{[]string{USER_AGENT_ROLE_ID}})

	req, _ := http.NewRequest("DELETE", endpoint, bytes.NewBuffer(b))
	req.Header.Set("content-type", "application/json")
	req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", apiToken))
	...
	if res.StatusCode != 204 {
		...
		fmt.Println(string(b))
		return errors.New("Failed to delete role on Auth0")
	}

	b2, _ := json.Marshal(&RoleRequest{[]string{OWNER_ROLE_ID}})

	req2, _ := http.NewRequest("POST", endpoint, bytes.NewBuffer(b2))
	req2.Header.Set("content-type", "application/json")
	req2.Header.Set("Authorization", fmt.Sprintf("Bearer %s", apiToken))
	...
	if res2.StatusCode != 204 {
		...
		return errors.New("Failed to add role on Auth0")
	}
